...
TEST_KWARGS_PRIORITY = {
    notify.ATTR_MESSAGE: "Test message",
    notify.ATTR_PRIORITY: 5,
}
...
class MockNotifyEntity(MockEntity, NotifyEntity):
    ...
    async def async_send_message(
        ...
        self.send_message_mock_calls(message, title=title, priority=priority)
...
@pytest.mark.parametrize(
    ...
    [
        MockNotifyEntityNonAsync(
            name="test",
            entity_id="notify.test",
            supported_features=NotifyEntityFeature.PRIORITY,
        ),
        MockNotifyEntity(
            name="test",
            entity_id="notify.test",
...
async def test_send_message_service_with_metadata(
    ...
    await hass.services.async_call(
        DOMAIN,
        SERVICE_SEND_MESSAGE,
        copy.deepcopy(TEST_KWARGS_PRIORITY) | {"entity_id": "notify.test"},
        blocking=True,
    )
    await hass.async_block_till_done()

    entity.send_message_mock_calls.assert_called_once_with(
        TEST_KWARGS_PRIORITY[notify.ATTR_MESSAGE],
        priority=TEST_KWARGS_PRIORITY[notify.ATTR_PRIORITY],
        title=None,
    )
    entity.send_message_mock_calls.reset_mock()
